...
if [[ -d "${AD_RSS_BASENAME}-install" ]] ; then
  ...
  cmake -G "Ninja" \
      -D_GLIBCXX_USE_CXX11_ABI=0 -DCMAKE_BUILD_TYPE=RelWithDebInfo -DBUILD_SHARED_LIBS=OFF -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DBUILD_TESTING=OFF -DCMAKE_INSTALL_PREFIX="../${AD_RSS_BASENAME}-install" \
