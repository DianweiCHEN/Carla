...
namespace s11n {
  ...
  /// increasing steadily.
  class ObjectLidarMeasurement {
    static_assert(sizeof(float) == sizeof(uint32_t), "Invalid float size");

    friend class ObjectLidarSerializer;
    friend class ObjectLidarHeaderView;
  ...
  public:
    ...
    explicit ObjectLidarMeasurement(uint32_t ChannelCount = 0u)
      : _header(Index::SIZE + ChannelCount, 0u) {
      _header[Index::ChannelCount] = ChannelCount;
    }

    ObjectLidarMeasurement &operator=(ObjectLidarMeasurement &&) = default;
    ...
    void Reset(uint32_t total_point_count) {
      ...
      _points.clear();
      _points.reserve(7u * total_point_count);
    }

    void WritePoint(uint32_t channel, rpc::Location point, rpc::Vector3D velocity, float intensity, uint32_t object_id) {
      ...
      _points.emplace_back(intensity);
      _points.emplace_back(static_cast<float>(object_id));
      _points.emplace_back(velocity.x);
      _points.emplace_back(velocity.y);
    }
