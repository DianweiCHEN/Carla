...
carla::rpc::VehicleControl RssRestrictor::restrictVehicleControl(
  ...
  // Pretty basic implementation of a RSS restrictor modifying the VehicleControl according to the given
  ...
  if (restriction.longitudinalRange.maximum < zeroAccel) {
    ...
    double sumBrakeTorque = mass * std::fabs(static_cast<double>(restriction.longitudinalRange.minimum)) * radius / 100.0;
    restrictedVehicleControl.brake = std::min(static_cast<float>(sumBrakeTorque / sumMaxBrakeTorque), 1.0f);
  }
